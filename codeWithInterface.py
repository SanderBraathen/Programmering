# -*- coding: utf-8 -*-
"""
Created on Thu Nov 16 18:26:32 2023
...
def enhance_ui(root):
    root.title("Weather-Based 3D Model Generator")
    ...
    # Header
    header = ttk.Label(root, text="3D Model Generator", font=("Helvetica", 16))
    header.pack(pady=10)

    # City Entry
    city_frame = ttk.Frame(root)
    city_frame.pack(pady=5, padx=10, fill=tk.X)

    ttk.Label(city_frame, text="Enter a city in Norway:").pack(side=tk.LEFT, padx=5)
    city_entry = ttk.Entry(city_frame)
    city_entry.pack(side=tk.RIGHT, expand=True, fill=tk.X, padx=5)

    # Generate Button
    generate_button = ttk.Button(root, text="Generate Model", command=lambda: on_submit(city_entry))
    generate_button.pack(pady=15)

    return city_entry

# Function to generate the model based on the weather
...
# Modify the submit function to accept the entry widget
def on_submit(entry_widget):
    city = entry_widget.get()
    if city:
        threading.Thread(target=generate_model, args=(city,)).start()
    else:
        messagebox.showerror("Error", "Please enter a city name.")

# Tkinter UI setup
root = tk.Tk()
city_entry = enhance_ui(root)  # Call the function to enhance UI
root.mainloop()
